...
def init_db():
    # Удаляем старую базу, если она существует (если вы запускаете скрипт заново)
    # import os
    # if os.path.exists('users.db'):
    #     os.remove('users.db') # <-- Опционально, если хотите удалять вручную

    if not os.path.exists('users.db'): # <-- Проверка останется, но файл будет создан с новой структурой
        ...
        ''')
        # Таблица трат с НОВЫМИ полями
        # ИСПРАВЛЕНО: Убрана лишняя запятая перед FOREIGN KEY
        cursor.execute('''
...
@app.route('/')
def index():
    with open('D:/артём/clown project/clown-project/clown project/clown-project/VibeFinance/main.html', 'r', encoding='utf-8') as f:
        return f.read()
...
def handle_message():
    ...
    # amount = data.get('amount') # <-- УБРАТЬ ЭТУ СТРОКУ, если она где-то ещё используется внутри функции
    ...
    # Попробуем обработать как трату
    initial_state = {"original_description": message, "user_id": user_id}
    try:
        final_state = spending_graph.invoke(initial_state)

        # Если мы дошли до этой точки, classify_spending не выбросил ошибку
        # Значит, это, скорее всего, трата
        ...
        try:
            ...
            ''', (
                ...
                psych_cat,
                amount,
                parsed_date, # <-- Сохраняем parsed_date в timestamp
                original_description,
                parsed_description,
                confidence,
                parsed_date # <-- Сохраняем parsed_date в новое поле
            ))
            conn.commit()
            conn.close()
        except Exception as e:
        ...
        # Возвращаем успешный ответ для траты
        return jsonify({
            'success': True,
            'classification': f"{main_cat} | {psych_cat}",
            'advice': advice, # <-- Ответ ИИ, основанный на трате
            'amount': amount,
        ...
        # classify_spending выбросил ValueError (например, не нашёл сумму)
        # Считаем, что это НЕ трата
        print(f"Message not recognized as spending (ValueError): {ve}. Treating as general query.")
        conn.close() # Закрываем соединение, т.к. трату не сохраняем
    except Exception as e:
        # Другие ошибки при вызове workflow
    ...
    try:
        ...
        # ВАЖНО: Убедитесь, что у вас есть метод respond_to_general_query в FinancialPsychologistAgent
